...
Cosmology is an empirical science. The fact that we have only one Universe to observe
...
strong isotropy in the Universe. If we combine the isotropic principle with the 
...
unit sphere. $f_K(\chi)$ is the comoving angular diameter distance that depends 
...
This metric is written in co-moving coordinate as the Universe is expanding and
therefore the fundamental observers coincides with the comoving observer.
...
\begin{equation}
	c^2\mathrm{d}d^2 = a^2\mathrm{d}\chi^2 \Rightarrow c\mathrm{d}t = -a(t)\mathrm{d}\chi
\end{equation}
\\
...
today by integrating this equation,
\begin{equation}
	\chi(t) = \int_t^{t_0} = \dfrac{c\mathrm{d}t^{\prime}}{a(t^{\prime})}
\end{equation}
\\
This expression relates the cosmic time to the comoving distance to a source
that we observe today. The comoving distance to the source can also be 
related to the scale factor and redshift as,
...
a factor of $a$ when the light was emitted. One example of a structure whose 
physical size we know is the {\it Baryon Accoustic Oscillation} (BAO) peak. The 
scale of the BAO peak is nearly $100 h^{-1} \mathrm{Mpc}$ and is considered as
a {\it standard ruler} in the Universe. It is one of the main cosmological science
driver in galaxy redshift surveys and a very promising tool to put strong constraints
on cosmology. For a thorough review see \cite{}.
...
expansion of the Universe.  To measure the luminosity distance to a source, we need to 
...
reduces the components of the field equation into two independent dynamical equations
...
parameters. This is called the concordance model.

\subsection{Equation of state of dark-energy}

Consider the second Friedmann equation (equation \ref{eqn:fe2} 
also known as the equation of motion for the 
Universe); in order to have an accelerated expansion of the Universe $\ddot{a}>0$,
